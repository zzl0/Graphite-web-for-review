...
def treatNoneAs(requestContext, seriesList, value=0):
  """
  Takes one metric or a wildcard seriesList.
  Continues the line with the last received value when gaps ('None' values) appear in your data, rather than breaking your line.
  ...
  .. code-block:: none
    ...
    &target=treatNoneAs(Server01.connections.handled, 0)

  """
  for series in seriesList:
    series.name = "treatNoneAs(%s, %s)" % (series.name, value)
    for i,value in enumerate(series):
      if value is None:
        value = 0
      series[i] = int(value)
...
def smooth(requestContext, seriesList, intervalString='5min'):
  ...
  for series in seriesList:
    ...
    values_per_point = int(interval/series.step)
...
def only_burst(requestContext, seriesList, threshold=1):
  def _all_greater_than(values, value):
    for val in values:
  ...
  for series in seriesList:
    ...
    for i, val in enumerate(series):
      #if _all_greater_than([prev, val], threshold):
    ...
    new_series = TimeSeries(series.name, series.start, series.end, series.step,
        values)
    results.append(new_series)

  return results

def points(requestContext, seriesList, max_points):
  results = []
  for series in seriesList:
    ...
    if len(series) % max_points != 0:
      values_per_point = len(series) // max_points + 1
    else:
      values_per_point = len(series) // max_points
    series.consolidate(values_per_point)
    series.step = series.step * values_per_point
    results.append(series)
  return results

def detectJump(requestContext, seriesList):
    ...
    def avg(l):
        not_none = [i for i in l if i is not None]
        if not not_none:
            return None
        return float(sum(not_none))/len(not_none)
    ...
    for series in seriesList:
        ...
        new_step = 300
        long_requestContext = requestContext.copy()
        long_requestContext['startTime'] = requestContext['startTime'] - one_hour
        long_series = evaluateTarget(long_requestContext, new_name)[0]
        if long_series.step < new_step:
            values_per_point = int(new_step/long_series.step)
            long_series.consolidate(values_per_point)
            new_name = "detectJump(%s)" % long_series.name
            long_series.step = new_step

        new_values = []
        points_to_skip = 12
        for i, val in enumerate(long_series[points_to_skip:], points_to_skip):
            if val is None:
                new_values.append(None)
                continue
            last_hour_median = median(long_series[i-points_to_skip:i])
            if last_hour_median is None or last_hour_median == 0:
                new_values.append(None)
                continue
            change_rate = int((float(val-last_hour_median))/last_hour_median*100)
            if abs(change_rate) > 10:
                new_values.append(change_rate)
            else:
                new_values.append(0)

        new_series = TimeSeries(new_name, series.start, series.end, new_step,
                new_values)
        new_series.pathExpression = new_name
        results.append(new_series)
    return results
...
def nPercentile(requestContext, seriesList, n):
  ...
  for s in seriesList:
    ...
    if perc_val:
      name = 'nPercentile(%s, %.1f)' % (s_copy.name, n)
