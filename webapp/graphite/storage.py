...
LOCAL_MEMCACHE = settings.LOCAL_MEMCACHE
...
KENSHIN_MC_KEY = 'kenshin:mckey:%s:%d:%d'
...
def find(root_dir, pattern):
  ...
  for absolute_path in _find(root_dir, pattern_parts):
    ...
    if isdir(absolute_path):
      ...
      if extension == '.hs' and kenshin:
        yield KenshinFile(absolute_path, metric_path)
...
class KenshinFile(Leaf):
  cached_context_data = None
  extension = '.hs'

  def __init__(self, *args, **kwargs):
    Leaf.__init__(self, *args, **kwargs)
    self._metric = self.real_metric
    self._real_path = realpath(self.fs_path)
  ...
  def fetch(self, startTime, endTime):
    start_time, end_time = int(startTime), int(endTime)
    start_interval = start_time / EXPIRE_TIME
    end_interval = end_time / EXPIRE_TIME

    key = KENSHIN_MC_KEY % (self._metric, start_interval, end_interval)
    data = mc.get(key)
    if not data:
      data = kenshin.fetch(self.fs_path, start_time, end_time)
      if not data:
        return None
      header, time_info, values = data
      values = np.array(values).transpose()
      tag_list = header['tag_list']
      min_step = header['archive_list'][0]['sec_per_point']
      for i, tag in enumerate(tag_list):
        key_tmp = KENSHIN_MC_KEY % (tag, start_interval, end_interval)
        val = (time_info, values[i].tolist(), min_step)
        mc.set(key_tmp, val, time=EXPIRE_TIME)
      data = (time_info, values[tag_list.index(self._metric)], min_step)
    return data
